            ...
            if (FabricDataGeneratorAccessor.class.isAssignableFrom(fabricDataGenerator.getClass())) {
                ...
                NexusServices.PLATFORM_MANAGER.getModData().forEach(curModData -> {
                    ...
                    if (allowDatagenForMod) {
                        ...
                        addProvider(modSpecificPack, !disabledProviders.contains(NexusProviderTypes.BLOCK_STATE_PROVIDER) && onClient, new StandardBlockStateProvider(modSpecificPackOutput, modId, providersToValidate.contains(NexusProviderTypes.BLOCK_STATE_PROVIDER), mappedDupeStrats.getOrDefault(NexusProviderTypes.BLOCK_STATE_PROVIDER, DuplicateDataPolicy.CRASH)));

                        addProvider(modSpecificPack, !disabledProviders.contains(NexusProviderTypes.SOUND_DEFINITION_PROVIDER) && onClient, new StandardSoundDefinitionProvider(modSpecificPackOutput, modId, providersToValidate.contains(NexusProviderTypes.SOUND_DEFINITION_PROVIDER), mappedDupeStrats.getOrDefault(NexusProviderTypes.SOUND_DEFINITION_PROVIDER, DuplicateDataPolicy.CRASH)));
