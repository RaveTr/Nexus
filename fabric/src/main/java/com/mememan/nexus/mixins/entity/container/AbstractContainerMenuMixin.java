...
/**
 * Mixin {@code class} intended to replace Vanilla's hardcoded block reach distance for BE menus with Nexus' own
...
public abstract class AbstractContainerMenuMixin {
    ...
    private AbstractContainerMenuMixin() {
        throw new IllegalArgumentException("Attempted to construct Mixin Class! (AbstractContainerMenuMixin)");
    }
    ...
    @ModifyExpressionValue(method = "method_17696", at = @At("MIXINEXTRAS:EXPRESSION"))
    private static boolean nexus$modifyHardcodedVanillaBlockReachRequirement(boolean original, @Local(argsOnly = true) Player player, @Local(argsOnly = true) BlockPos targetPos) {
