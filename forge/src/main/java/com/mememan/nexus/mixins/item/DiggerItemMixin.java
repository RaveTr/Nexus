 ...
 * sense to have a redundant mixin that applies to both loaders from common into an unused target.
 *
...
public abstract class DiggerItemMixin {
    ...
    private boolean nexus$determineCorrectMiningLevelForDrops(boolean original, ItemStack targetStack, BlockState targetState) {
        AtomicReference<OptionalBoolean> modifiedResult = new AtomicReference<>(OptionalBoolean.empty());
        ...
        PropertyWrapper.getMappedPropertyWrappers().entrySet().stream()
                ...
                .ifPresent(mappedBPW -> {
                    ...
                    if (Math.abs(minMiningLevel) > 0) {
                        ...
                        modifiedResult.set(OptionalBoolean.of(curMiningLevel > minMiningLevel));
                    }
                });

        return original && (!modifiedResult.get().isPresent() || modifiedResult.get().getAsBoolean());
    }
}
